...
case class CreateTrainingMenuUseCase @Inject()(repository: TrainingMenuRepository)(userService: UserService) (implicit ec: ExecutionContext){
  def apply(uid: FirebaseUid)(trainingMenuRequestModel: TrainingMenuRequestModel): Future[Either[Error, TrainingMenuResponseModel]] =
    userService.getUser(uid).flatMap {
      case None => Future.successful(Left(EntityNotFoundError()))
      case Some(user) => trainingMenuRequestModel.toEntity(user.id) match {
